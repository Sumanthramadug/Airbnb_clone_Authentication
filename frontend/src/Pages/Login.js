...
const Login = () => {
    ...
    const submitHandler = async(e)=>{
        ...
        if(state === 'SignUp'){
            try{
                const {data}= await axios.post('https://airbnb-clone-3xan.onrender.com/users/register',{name,email,password},{withCredentials:true});
                if(data.success){
            ...
            const {data} = await axios.post('https://airbnb-clone-3xan.onrender.com/users/login',{email,password},{withCredentials:true});
            try{
                if(data.success){
                    navigate("/");
                    toast.success(data.message);
